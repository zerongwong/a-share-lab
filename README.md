...
收盘后先分阶段同步，次日 06:30、07:30、08:00 再补齐并核验；08:18前收尾释放数据锁，08:18–09:00不启动新同步。
盘前报告在北京时间周一至周五08:20 首次尝试，08:30、08:40、08:50 有限重试；自动计算与提交窗口为08:20–08:58（不含08:58），目标09:00前收到。
独立截止检查在08:58、08:59运行，不争用主报告运行锁；正式计划未确认受理时有限尝试提交精简状态，不能冒称已有买入结论。
程序还必须确认当天是否交易日，节假日不按普通工作日猜测；只有上一完整交易日的共同截止和质量全部合格，才形成当天计划。
已记录受理的文字会去重；网络结果未知时可有限重试，不能保证第三方严格仅送达一次，也不把未知当作已送达。

日常微信仅保留盘前报告、15:30–16:00的已登记持仓盈亏日报，以及盘中触发止损等风险例外。
持仓日报在15:35、15:45、15:55有限尝试，无有效登记持仓不虚构日报；数据同步、恢复和旧方案到期复盘只保留本地记录，不再单独推送。
...
安装计划任务需使用者主动执行。报告安装脚本会同时登记主报告、独立截止检查和持仓日报三个任务，失败恢复原任务；停止时统一卸载三个任务，不删除研究数据：
