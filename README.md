...
1. 用共同截止日、交易资格、流动性和市场/核心指数状态作硬门；
2. 只保留早期或有序主升趋势；
...
6. 分别搜索 3、4、5 只组合，按“逆下行波动 × 有界信号倾斜”自动分配权重；
7. 同时约束年化下行波动、60 日滚动回撤严重度 P90、5 日 ES95、下跌相关性、
   单股下行风险贡献和行业集中度；
8. 风险预算内比较历史持有期收益下界；全市场覆盖或证据不足就返回空结果或
   `VALIDATION_NOT_READY`，证据齐全也只称 `RESEARCH_ONLY`。

4 只为注意力与分散的常态；没有任何四股组合通过全部风险预算时才退到 3 只并把股票
敞口降至 70%。5 只只有在历史收益下界更高、路径与尾部风险不恶化且下跌相关性或单股
风险贡献得到实质改善时，才替代四股常态。融资始终为 0，不为凑数买次优股票。

完整公式、Edwards–Magee 与 Livermore 的使用位置、证据否决层和验证要求见
[中期选股与组合方法](docs/STRATEGY.md)，程序分层见 [架构](docs/ARCHITECTURE.md)，
第三方数据边界见 [数据与许可政策](docs/DATA_POLICY.md)。

## 旧版固定四股原型（仅保留兼容，不是主入口）

以下内容记录了早期 `3:2:2:1` 固定四股原型的实际方法和限制，便于复核旧档案与兼容
测试。当前网页和 MCP 的 `generate_portfolio` 已改用上面的动态 3–5 股主模型。
...
- Server酱 Turbo：向个人微信发送简短提醒；
- Bark：向 iPhone 发送完整的本地研究行动摘要。

SendKey 和 Bark 设备 Key 只通过密码输入框在本机录入，并保存到 macOS 钥匙串。项目不会把
它们写入 `.env`、TOML、SQLite、日志或 GitHub。设置页可分别发送无股票内容的测试消息；
两个通道相互独立，一个失败不会阻止另一个通道发送。

当前页面只完成凭据保存与连通性测试，尚未启用每日定时任务。未来通知只发送衍生结论，
不发送 CSMAR 原始数据、API 凭据或完整持仓金额。Server酱和 Bark 都是外部服务，使用前
应阅读其隐私条款；需要最高隐私时应自建 Bark 服务端。
