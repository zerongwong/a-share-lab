...
当前盘前报告与连续组合服务使用 `continuous-signal-v2`：**不设强制到期卖出**，
...
初建从可用市场数据逐股筛选，再取排名前 36 个候选，用宽度 128 的近似搜索比较 1–8 股方案与 0 股现金基准；
...
只有使用者下一次明确确认真实建仓，才创建新 revision、新 `position_key` 与隔离的 v2 追踪身份；
...
盘前报告在北京时间周一至周五09:00 首次尝试，09:10、09:20 有限重试，并在 09:30 开盘前关闭自动提交窗口。
