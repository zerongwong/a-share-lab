...
3. 以共同截止日全市场宽度计算组合级大盘风险门，再计算风险调整收益、回撤、趋势、
   流动性、行业与相关性约束；
4. 只有在完整、点时覆盖时才启用财务、新闻和板块因子；大盘环境不作为单只股票加分项；
5. 在合格集合中生成四股组合，并按 `3:2:2:1` 分配股票仓；
6. 展示历史 CAGR、波动、Sharpe、Sortino、Calmar、回撤分布和收益情景；
7. 可选地把研究结论写入本机不可篡改的 SQLite 档案。
...
- `generate_portfolio`：临时生成四股组合，不写档案、不返回原始数据；
