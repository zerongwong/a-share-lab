...
此功能默认关闭。使用者明确授权后，程序每天只把当前仍有效持仓的规范化六位股票代码提交给
巨潮资讯公开接口，用于核验除权、现金分红、送股、转增和配股。应用层不会提交股票名称、
入场日、成本、股数、金额、权重、组合编号或保护线；统一公共查询起点和当天日期属于接口协议，
并非账户数据。网络服务仍会像普通 HTTPS 访问一样看到调用 IP 等连接元数据。

巨潮原始响应不进入 GitHub、通知或运行日志。本机只追加保存代码、持仓版本身份、三态结论
（`CLEAR / DETECTED / UNKNOWN`）、覆盖区间、知识时间、事件日期/类别、完整性摘要与不可逆哈希。
空持仓、未授权、授权撤销或持仓超过已验证的 5 只范围时不发请求；查询期间持仓版本变化则丢弃结果。
任何失败都收敛为 `UNKNOWN`，只阻止破位被误写成已确认退出，不阻断行情同步、候选研究或文字晚报。
临时 `UNKNOWN` 以 15 分钟退避、每个盘中/收盘阶段最多尝试 3 次；成功或已发现事件后当天该阶段不再查询。
若确实发现持仓期内公司行动，系统不会自行修改成本或股数；该持仓保持“待人工核对”，直至使用者明确提供
公司行动后的新基准，避免把除权形成的价格缺口误判成真实亏损。
