...
```mermaid
flowchart LR
    D["合法数据＋共同截止"] --> G["早期转强＋突破/健康回踩硬门"]
    G --> C["1–8 股候选组合＋0 股现金基准"]
    C --> R["周期敞口＋组合风险门"]
    R --> P["0–8 股连续计划<br/>5–6 只常态优先，不凑数"]
    P --> H["用户确认实际建仓<br/>新 revision＋position_key＋v2 追踪身份"]
    H --> V["完整确认账户估值快照<br/>启动真实绩效链"]
    H --> M["收盘管理＋盘中风险监控"]
```

- 0 只表示现金；1–3 只是低总敞口的集中过渡状态；4–8 只为成型组合；5–6 只是常态优先区间。
- 每行业最多一只；每个新建仓名义权重不得超过总账户 20%，约 15% 是研究中枢而非强制等权。
- v2 迁移只改数据结构，不推断成交或自动停用旧持仓；开源使用者须在确认空仓后显式写入 `cleared` revision。本机这次已完成该显式清空，旧持仓、通知、决策和绩效只读保留。在用户明确确认下一次真实建仓前，程序不从推荐推断活跃仓位。
- 初建仍使用股票仓内 10% 档后换算总账户权重；单补位枚举总账户 10%/20% 档。两者都须通过 20% 单股硬上限。
...
当前固定期限接口只作只读历史兼容；自 2026-09-18 起不再由每日同步更新、结算或通知，
也不能建立或跟踪持仓。生产连续计划统一在交易日 09:00 盘前窗口发送。
...
- 当前 `zero_budget_eod` 链按来源、未复权口径和交易日写入独立 `market_overlay`；旧
  Infoway 链保留审计但不与当前链混用；
...
- Tushare 股票成交量由“手”转换为“股”、成交额由千元转换为人民币元；BaoStock 指数
  使用股和人民币元。两者都逐行验证量额隐含成交价，股票再由 AKShare 独立抽样复核；
- 当前自动母集来自 BaoStock 的沪深上市 A 股清单，不包含北交所、指数和 B 股；停牌导致
  的少量缺行由统一 98% 覆盖门处理；
