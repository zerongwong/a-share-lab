...
> 旧版说明，不是当前连续组合生产合同。本文保留六期限方法及其历史实现声明，
> “当前”“主模型”等措辞仅指旧版当时状态；请先读[连续组合合同](CONTINUOUS_STRATEGY.md)。
> 新版采用 `continuous-signal-v3` 单组、无强制到期；不得按本文重解释新版或改写旧档案。
> 本文所有六期限 3–5 股规则都是只读兼容语义，不是当前生产搜索边界。

## 当前连续策略摘要（v2）

- 允许 0–8 只：0 只是现金，1–3 只是降低总敞口的集中过渡状态，4–8 只是成型组合，5–6 只是常态优先区间。
- 不为凑数放宽证据、结构或风险门；每行业最多一只。
- 每个新建仓名义权重的总账户硬上限为 20%，约 15% 是研究中枢，不是强制每只精确 15%。
- v2 数据库升级不推断成交、不自动停用旧活跃跟踪；开源使用者需在确认空仓后显式写入 `cleared` revision。本机这次已完成该显式清空，历史持仓、通知、决策和绩效审计保留不改写。下一次明确确认的真实建仓才创建新 revision、新 `position_key` 和 v2 追踪身份；真实绩效链还须从完整确认的账户估值快照建立。
...
当前自动通知采用“收盘后分阶段同步、次日 06:30–08:40 补齐、09:00 当日盘前研究报告”的顺序。
连续计划与去重、安全边界见 [09:00 当日盘前研究报告](EVENING_REPORT.md)，旧期限到期日、三榜与
未复权收益口径见 [六周期推荐的滚动到期复盘](PERFORMANCE_TRACKING.md)。

旧六期限月度复盘已于 2026-09-18 退出生产自动任务；只有明确运行本地历史审计时，才会按
三类档案和六期限分别汇总，并逐行按当时可知时间过滤。样本不足、基准不可用或档案扫描不完整
时不下方向性结论。负收益或弱于可靠同区间基准只会形成版本化实验建议，必须经使用者讨论批准
并重新完成 purged walk-forward 才能更新正式规则，详见
[月度模型复盘](MONTHLY_MODEL_REVIEW.md)。
